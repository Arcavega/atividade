...
function showFilter() {
    ...
    console.log(filtro.style.visibility)
    let button = document.getElementById("showFil");
    const pIf = document.getElementById("if");

    if (filtro.style.visibility == 'visible'){
        filtro.style.visibility = 'hidden';
        filtro.style.overflowX = 'hidden';
        filtro.style.width = '0px';
        button.innerHTML = '<'
        console.log("ola")
    } else{
        console.log('Não')
        filtro.style.visibility = 'visible';
        filtro.style.overflowX = 'visible';
        filtro.style.width = '350px';
        button.innerHTML = '>'
    }
}
